/**
 * Mass CTV launcher — 200 concurrent sessions, unique proxy + fingerprint per session.
 * Cycles through fingerprints until proxies are exhausted.
 *
 * - Fingerprints: data/fingerprints_tv.csv (10,000)
 * - Proxies: data/proxies.txt (1 per session, consumed once)
 * - App rotation: data/app_rotation.csv (round-robin)
 * - Successful bids → logs/mass-<ts>/bids.csv
 * - Per-session logs → logs/mass-<ts>/sessions/
 *
 * Usage: node --import tsx scripts/launch-mass.ts
 */
...
if (fingerprints.length === 0) {
  console.error('ERROR: No fingerprints found in data/fingerprints_tv.csv');
  process.exit(1);
}
...
if (proxies.length === 0) {
  console.error('ERROR: No proxies found in data/proxies.txt');
  process.exit(1);
}

const apps = loadAppRotation();
// Fallback if app_rotation.csv is empty
const DEFAULT_APP = {
  bundle: 'tv.pluto.android',
  name: 'Pluto TV',
  storeurl: 'https://play.google.com/store/apps/details?id=tv.pluto.android',
  ver: '5.40.1',
};

const totalSessions = Math.min(fingerprints.length, proxies.length);

// ── Output directories ──────────────────────────────────────────────────────
const TS = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
const BASE_DIR = resolve(process.cwd(), 'logs', `mass-${TS}`);
const SESSION_DIR = resolve(BASE_DIR, 'sessions');
mkdirSync(SESSION_DIR, { recursive: true });

// CSV bid log
const csvPath = resolve(BASE_DIR, 'bids.csv');
const csvStream = createWriteStream(csvPath);
csvStream.write('timestamp,store_url,bundle,carrier,user_agent,os,model,vendor,ip,bid_price,geo_city,geo_region,geo_country,proxy_ip,session_index\n');

// Summary log
const summaryPath = resolve(BASE_DIR, 'summary.log');
...
async function batchGeoLookup(ips: string[]): Promise<void> {
  ...
  for (let i = 0; i < uncached.length; i += 100) {
    ...
    // ip-api.com rate limit: 15 req/min for batch — small pause between chunks
...
async function runSession(index: number): Promise<void> {
  ...
  const device: DeviceProfile = { ...fingerprints[index] };
  ...
  try {
    ...
    completedSessions++;
    const sessionLog = {
      index: index + 1, status: 'PROXY_FAIL', os: device.os, model: device.model,
      vendor: device.vendor, error: `proxy unreachable: ${(err as Error).message}`,
    };
    writeFileSync(resolve(SESSION_DIR, `${num}_proxy-fail.json`), JSON.stringify(sessionLog, null, 2));
    log(`  [${num}] PROXY_FAIL ${device.os} ${device.model}`);
    return;
  }

  // ── Override device with proxy data ──
  device.ip = proxyIp;
  ...
  log(`  [${num}] ${status.padEnd(6)} ${device.os.padEnd(10)} ${device.vendor.padEnd(8)} ${device.model.padEnd(22)} ${proxyIp.padEnd(16)} ${city.padEnd(16)} ${latencyMs}ms ${price}`);

  // ── CSV logging for successful bids ──
  if (hasBid && bidResult) {
    csvStream.write([
      new Date().toISOString(),
      csvEsc(app.storeurl),
      app.bundle,
      csvEsc(device.carrier || ''),
      csvEsc(device.userAgent),
      device.os,
      device.model,
      device.vendor,
      proxyIp,
      bidResult.auctionData.price.toFixed(4),
      city,
      region,
      device.geo?.country || '',
      proxyIp,
  ...
  const fileName = `${num}_${status}_${sanitize(device.os)}_${sanitize(device.vendor)}_${sanitize(device.model)}_${sanitize(proxyIp)}`;
  ...
  writeFileSync(resolve(SESSION_DIR, `${fileName}.json`), JSON.stringify(sessionDump, null, 2));

  // Save VAST separately for bids
  if (hasBid && bidResult) {
...
async function main(): Promise<void> {
  ...
  log('═══════════════════════════════════════════════════════════════');
  log('  CTV MASS LAUNCHER');
  log('═══════════════════════════════════════════════════════════════');
  log(`  Fingerprints:  ${fingerprints.length}`);
  log(`  Proxies:       ${proxies.length}`);
  log(`  Apps:          ${apps.length || 1} (${apps.length ? 'from CSV' : 'default'})`);
  ...
  // ── Phase 1: Resolve proxy IPs in parallel ──
  ...
  while (cursor < totalSessions || running.size > 0) {
    ...
    if (completedSessions > 0 && completedSessions % 50 === 0) {

...
async function createSession(sessionId, config) {
    ...
    try {
        ...
        const { page } = await browserPool.createContext(sessionId, config.payload.device, config.payload.networkEmulation);
        setupNetworkInterceptor(page, sessionId, (entry) => {
        ...
        try {
            ...
            if (sm.canRetry()) {
                ...
                sm.setError(SessionState.ERROR_NETWORK, err.message);
        ...
        try {
            ...
            sm.setError(SessionState.ERROR_VAST, err.message);
            sendToMaster({ type: 'session-error', sessionId, error: sm.error, state: sm.state });
            return;
        }
        // AD LOADING
        ...
        sendToMaster({ type: 'session-update', sessionId, state: sm.state });
        try {
            await page.evaluate((url) => window.__loadAd(url), creative.mediaUrl);
        }
        catch (err) {
            if (sm.canRetry()) {
                sm.incrementRetry();
                try {
                    ...
                    sm.setError(SessionState.ERROR_MEDIA, err.message);
                    sendToMaster({ type: 'session-error', sessionId, error: sm.error, state: sm.state });
                    return;
                }
            }
        ...
        await page.waitForFunction(() => window.__adCompleted === true, { timeout: (creative.duration + 10) * 1000 });
        // CONTENT PLAYING
        sm.transition(SessionState.CONTENT_PLAYING);
        sendToMaster({ type: 'session-update', sessionId, state: sm.state });
        await page.evaluate((url) => window.__loadContent(url), config.payload.contentUrl);
        await page.waitForTimeout(5000);
        // STOPPING
        sm.transition(SessionState.STOPPING);
        await page.evaluate(() => window.__stopAll());
        ...
        sendToMaster({ type: 'session-error', sessionId, error: err.message, state: sm.state });
        await browserPool.closeContext(sessionId);
    }
    finally {
